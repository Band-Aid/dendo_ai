...
function conceptBlock(
  ...
  if (measures.length) {
    const shown = measures
      ...
      .join(', ')
    const more = measures.length > MAX_ITEMS ? ` (+${measures.length - MAX_ITEMS} more — lookup_ontology)` : ''
    lines.push(`  Measures: ${shown}${more}`)
  }

  for (const cause of concept.causes.slice(0, MAX_ITEMS)) {
  ...
  if (concept.actions.length) {
    lines.push(`  Actions: ${concept.actions.slice(0, MAX_ITEMS).map(a => a.title).join('; ')}`)
  }
 ...
 */
export function buildOntologyDigest(orgId: string): string | null {
